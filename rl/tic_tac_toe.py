...
class EpsAgent(Agent):
    def __init__(self, eps=1, alpha=1):
    ...
    def take_action(self, env):
        ...
        r = np.random.rand()
        best_state = None
        if r < self.eps / math.sqrt(self.epochs):
...
class OIVAgent(Agent):
    def __init__(self, upper_limit=10, alpha=0.5):
        self.upper_limit = upper_limit  # probability of choosing random action instead of greedy
        self.alpha = alpha  # learning rate
        self.verbose = False
        self.state_history = []
        self.epochs = 1
